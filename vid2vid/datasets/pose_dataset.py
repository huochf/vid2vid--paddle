...
from vid2vid.datasets.keypoint2img import read_keypoints
...
class PoseDataset(BaseDataset):
    ...
    def initialize(self, cfg):
        self.cfg = cfg
        self.pose_type = cfg.pose_type

        root = cfg.dataroot
        if cfg.isTrain:
            self.img_paths = sorted(make_grouped_dataset(path.join(root, 'images')))
            self.op_paths = sorted(make_grouped_dataset(path.join(root, 'openpose')))
            self.dp_paths = sorted(make_grouped_dataset(path.join(root, 'densepose')))
            ...
            self.img_paths = sorted(make_grouped_dataset(cfg.seq_path))
            self.op_paths = sorted(make_grouped_dataset(cfg.seq_path.replace('images', 'openpose')))
            self.dp_paths = sorted(make_grouped_dataset(cfg.seq_path.replace('images', 'densepose')))

            self.ref_img_paths = sorted(make_grouped_dataset(cfg.ref_img_path))
            self.ref_op_paths = sorted(make_grouped_dataset(cfg.ref_img_path.replace('images', 'openpose')))
            self.ref_dp_paths = sorted(make_grouped_dataset(cfg.ref_img_path.replace('images', 'densepose')))
        ...
        self.ref_crop_coords = [None] * cfg.n_shot
        self.seq_max_length = max(10000, max([len(A) for A in self.img_paths]))  # max number of frames in the training sequences
    ...
    def set_inference_sequence_idx(self, sequence_idx, k_shot_index, k_shot_frame_index):
        ...
        # assert k_shot_frame_index < len(self.img_paths[k_shot_index])
    ...
    def get_current_sequence_length(self, ):
        return len(self.img_paths[self.inference_sequence_idx])
    ...
    def get_items(self, index):
        ...
        if cfg.isTrain:
            ...
            ref_img_paths = self.ref_img_paths[seq_idx]
            ref_op_paths = self.ref_op_paths[seq_idx]
            ref_dp_paths = self.dp_paths[seq_idx]
        ...
        n_frames_total, start_idx, t_step, ref_indices = get_video_params(self.cfg, self.n_frames_total, len(img_paths), index)
        ...
        if is_first_frame: # need to read reference images for every training iter or at beginning of inference            
            ...
            if not cfg.isTrain: # keep track of non-changing variables during inference
        ...
        crop_coords = self.crop_coords if not cfg.isTrain else ref_crop_coords[0]        
    ...
    def get_image(self, A_path, size, params, crop_coords, input_type, ppl_idx=None, op=None, ref_face_pts=None):
        ...
        if input_type == 'openpose':
            ...
            A_img, pose_pts, face_pts = read_keypoints(cfg, A_path, size,
                cfg.basic_point_only, cfg.remove_face_labels, ppl_idx, ref_face_pts)
        ...
        transform_scaleA = get_transform(self.cfg, params, method=method, color_aug=is_img and cfg.isTrain)
        A_scaled = transform_scaleA(A_img, params).astype("float32")
    ...
    def crop_person_region(self, A_img, crop_coords, pose_pts=None, size=None):
        ...
        if crop_coords is None: 
            ...
            random_offset = [random.uniform(-offset_max, offset_max), 
                             random.uniform(-offset_max, offset_max)] if self.cfg.isTrain else [0,0]
    ...
    def get_crop_coords(self, pose_pts, size, offset=None):                
        ...
        scale = random.uniform(1.4, 1.6) if self.cfg.isTrain else 1.5

        # bh, bw: half of height / width of final cropped size        
        bh = int(min(h, max(h//4, y_len * scale))) // 2
        bw = int(bh * self.cfg.aspect_ratio)
    ...
    def __len__(self):        
        # if not self.cfg.isTrain: return 1 # len(self.img_paths)
