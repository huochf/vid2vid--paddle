...
from vid2vid.model.liteflownet.models import build_model
from vid2vid.model.model_utils import (get_face_mask, get_fg_mask,
                                       get_part_mask, pick_image, resample)


class DummyFlowNet(dg.Layer):

    def forward(self, x1, x2):
        return L.zeros_like(x1)[:, :2, :, :]


class MaskedL1Loss(dg.Layer):
    """
...
class FlowLoss(dg.Layer):
    ...
    def forward(self, data, net_G_output, current_epoch):
        ...
        Returns:
            (dict):
                ...
                - loss_flow_warp (tensor): L1 loss between the warped image and target image when using the flow to warp.
                - loss_mask (tensor): Loss for the occlusion masks.
        """
        ...
        # Compute the ground truth flows and confidense maps.
        ...
        with warnings.catch_warnings():
            ...
            if self.warp_ref:
                ...
                if self.for_pose_dataset:
                    ...
                    flow_gt_ref = self.flowNet(tgt_label[:, :3], ref_label[:, :3])
                else:
                    # Use RGB images for other datasets.
                    flow_gt_ref = self.flowNet(tgt_image, ref_image)
            ...
            if current_epoch >= self.cfg.single_frame_epoch and data['real_prev_image'] is not None:
                ...
                flow_gt_prev = self.flowNet(tgt_image, tgt_image_prev)
        ...
        loss_flow_L1, loss_flow_warp, body_mask_diff = self.compute_flow_losses(flow, warped_images, tgt_image, flow_gt,
        ...
        return loss_flow_L1, loss_flow_warp, loss_mask, flow_gt
    ...
    def compute_flow_losses(self, flow, warped_images, tgt_image, flow_gt, flow_conf_gt, fg_mask, tgt_label, ref_label):
        ...
        if isinstance(flow, list):
            ...
            for i in range(len(flow)):
                loss_flow_L1_i, loss_flow_warp_i = self.compute_flow_loss(flow[i], warped_images[i], tgt_image, flow_gt[i],
        ...
        if self.warp_ref:
            if self.for_pose_dataset:
                ...
                body_mask_diff = L.reduce_sum(L.abs(warped_ref_body_mask - body_mask), 1, keep_dim=True)
    ...
    def compute_flow_loss(self, flow, warped_image, tgt_image, flow_gt, flow_conf_gt, fg_mask):
        ...
        Returns:
            (dict):
                - loss_flow_L1 (tensor): L1 loss compared to ground truth flow.
                - loss_flow_wrap (tensor): L1 loss between the warped image and the 
                    target image when using the flow to warp.
        """
        loss_flow_L1 = dg.to_variable(np.zeros((1, )).astype("float32"))
        loss_flow_warp = dg.to_variable(np.zeros((1, )).astype("float32"))
        if flow is not None and flow_gt is not None:
            # L1 loss compared to flow ground truth
            loss_flow_L1 = self.criterionMasked(flow, flow_gt, fg_mask)
    ...
    def compute_mask_losses(self, occ_mask, fake_image, warped_image, tgt_label, tgt_image,
        ...
        if self.warp_ref:
            ...
            if self.for_pose_dataset:
                ...
                face_mask = L.pool2d(face_mask, pool_size=15, pool_type='avg', pool_stride=1, pool_padding=7)
    ...
    def compute_mask_loss(self, occ_mask, warped_image, tgt_image):
        ...
        if occ_mask is not None:
            ...
            img_diff = L.reduce_sum(L.abs(warped_image - tgt_image), 1, keep_dim=True)
