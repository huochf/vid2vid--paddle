...
class Discriminator(dg.Layer):
    ...
    def forward(self, data, net_G_output, real=True):
        ...
        Args:
            ...
            net_G_output (dict):
                fake_image (N x C1 x H x W tensor): Fake images.
        ...
        output_x = dict()
        if 'label' in data:
            fake_input_x = L.concat((data['label'], net_G_output['fake_images']), 1)
        else:
            fake_input_x = net_G_output['fake_images']
        
        output_x['fake_outputs'], output_x['fake_features'], _ = self.model.forward(fake_input_x)

        if real:
            if 'label' in data:
                real_input_x = L.concat((data['label'], data['images']), 1)
            else:
                real_input_x = data['images']
            output_x['real_outputs'], output_x['real_features'], _ = self.model.forward(real_input_x)
        return output_x


class MultiResPatchDiscriminator(dg.Layer):
    """
    Multi-resolution patch discriminator.
    ...
    def forward(self, input_x):
        ...
        Returns:
            ...
            - input_list (list): list of downsampled iinput images.
        """
        input_list = []
        output_list = []
        features_list = []
...
class WeightSharedMultiResPatchDiscriminator(dg.Layer):
    """
    ...
    """
    def __init__(self, num_discriminators=3, kernel_size=3, num_image_channels=3,
        num_filters=64, num_layers=4, max_num_filters=512, activation_norm_type='', weight_norm_type='', **kwargs):
        super().__init__()
        for key in kwargs:
            if key != 'type' and key != 'patch_wise':
                warnings.warn("Discriminator argument {} is not used.".format(key))
        ...
        self.discriminator = NLayerPatchDiscriminator(kernel_size, num_image_channels,
            num_filters, num_layers, max_num_filters, activation_norm_type, weight_norm_type)
        print("Done with the Weight-Shared Multi-resolution patch discriminator initialization.")
...
class NLayerPatchDiscriminator(dg.Layer):
    """
    ...
    Args:
        ...
        num_filters (int): Num. of base filters in a layer.
        num_layers (int): Num. of layers for the patch discriminator.
        max_num_filters (int): Msximum num. of filters in a layer.
        activation_norm_type (str): batch_norm/instance_norm/none/...
        weight_norm_type (str): none/spectral_norm/weight_norm
    """
