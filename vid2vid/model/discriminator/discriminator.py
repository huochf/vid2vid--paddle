...
from vid2vid.model.discriminator.multires_patch import NLayerPatchDiscriminator
from vid2vid.model.model_utils import get_fg_mask, pick_image
from vid2vid.utils.data import (get_paired_input_image_channel_number,
...
class Discriminator(dg.Layer):
    ...
    def forward(self, data, net_G_output, past_frames):
        ...
        if len(label.shape) == 5:
            label = label[:, -1]
        if self.use_few_shot:
            # Pick only one the latest output frame.
            ref_idx = net_G_output['ref_idx'] if 'ref_idx' in net_G_output else 0
            ...
            label = L.concat([label, ref_label, ref_image], dim=1)
        fake_image = net_G_output['fake_images']
        output = dict()

        # Individual frame loss.
        pred_real, pred_fake = self.discriminate_image(self.net_D, label, real_image, fake_image)
        output['indv'] = dict()
        output['indv']['pred_real'] = pred_real
        output['indv']['pred_fake'] = pred_fake

        if 'fake_raw_images' in net_G_output and net_G_output['fake_raw_images'] is not None:
            # Raw generator output loss.
            fake_raw_image = net_G_output['fake_raw_images']
            fg_mask = get_fg_mask(data['label'], self.has_fg)
            pred_real, pred_fake = self.discriminate_image(self.net_D, label, real_image * fg_mask, fake_raw_image * fg_mask)
            output['raw'] = dict()
        ...
        if self.add_dis_cfg is not None:
            for name in self.add_dis_cfg:
                # Crop corresponding regions in the image according to the crop function.
                add_dis_cfg = self.add_dis_cfg[name]
                ...
                if self.use_few_shot:
                    ...
                    if ref_crop is not None:
                        real_crop = L.concat([real_crop, ref_crop], 1)
                        fake_crop = L.concat([fake_crop, ref_crop], 1)
                
                # Feed the crops to specific discriminator.
                if fake_crop is not None:
                    net_D = getattr(self, 'net_D_' + name)
                    pred_real, pred_fake = self.discriminate_image(net_D, None, real_crop, fake_crop)
                else:
        ...
        # Temporal loss.
        past_frames, skipped_frames = get_all_skipped_frames(past_frames, [real_image, fake_image], self.num_scales, self.num_frames_D)

        for scale in range(self.num_scales):
            real_image, fake_image = [skipped_frame[scale] for skipped_frame in skipped_frames]
            pred_real, pred_fake = self.discriminate_video(real_image, fake_image, scale)
    ...
    def discriminate_image(self, net_D, real_A, real_B, fake_B):
        """
        ...
        if real_A is not None:
            real_AB = L.concat([real_A, real_B], 1)
            fake_AB = L.concat([real_A, fake_B], 1)
        else:
    ...
    def discriminate_video(self, real_B, fake_B, scale):
        ...
        real_B = L.reshape(real_B, (-1, self.num_netDT_input_channels, height, width))
        fake_B = L.reshape(fake_B, (-1, self.num_netDT_input_channels, height, width))
...
def get_all_skipped_frames(past_frames, new_frames, t_scales, tD):
    ...
    for past_frame, new_frame in zip(past_frames, new_frames):
        skipped_frame = None
        if t_scales > 0:
            past_frame, skipped_frame = get_skipped_frames(past_frame, L.unsqueeze(new_frame, [1]), t_scales, tD)
...
def get_skipped_frames(all_frames, frame, t_scales, tD):
    ...
    """
    all_frames = L.concat([all_frames, frame], 1) if all_frames is not None else frame
...
class MultiPatchDiscriminator(dg.Layer):
    """
    ...
    def __init__(self, dis_cfg, num_input_channels):
        ...
        num_discriminators = getattr(dis_cfg, 'num_discriminators', 3) # 2
        ...
        self.nets_discriminators = []
        for i in range(num_discriminators):
            net_discriminator = NLayerPatchDiscriminator(kernel_size, num_input_channels,
                num_filters, num_layers, max_num_filters, activation_norm_type, weight_norm_type)
            self.add_sublayer('discriminator_%d' % i, net_discriminator)
            self.nets_discriminators.append(net_discriminator)
